<script lang="ts">
	...
	function handleRemove(plantId: string) {
		...
		if (
			plant &&
			confirm(`Are you sure you want to remove "${plant.name}"? This action cannot be undone.`)
		) {
				...
				<div class="plant-card">
					...
					<div class="plant-info">
						...
						<div class="plant-actions">
							<button class="btn-secondary" onclick={() => handleEdit(plant.id)}>Edit</button>
							<button class="btn-danger" onclick={() => handleRemove(plant.id)}>Remove</button>
						</div>
					</div>
				</div>
			{/each}
	...
	{#if showAddForm}
		...
		<div class="modal-overlay" onclick={closeModals}>
			...
			<div class="modal-content" onclick={(e) => e.stopPropagation()}>
				...
				<AddPlantForm onClose={closeModals} />
			</div>
		</div>
	{/if}
